...
### Loading assests ###
img_path_mouse = 'C:\\Users\\Corentin offline\\Downloads\\cute_mouse.png'
img_path_key = 'C:\\Users\\Corentin offline\\Downloads\\key.png'
img_path_cheese = 'C:\\Users\\Corentin offline\\Downloads\\cheese.png'
agent_img = mpimg.imread(img_path_mouse)
cheese_img = mpimg.imread(img_path_cheese)
key_img = mpimg.imread(img_path_key)

### Defining environment ###
grid_dims = [5, 7]
num_grid_points = np.prod(grid_dims) # total number of grid locs
...
## Defining state-observation matrix A
...
#initializing a random A for learning from scratch
...
## Prior over preference and states
...
# defining the geneative process, or world, from which observations are generated
class GridWorldEnv():
...
### Run AI ###
# Create dirichlet param priors to allow learning of the environment.
pA = utils.dirichlet_like(A, scale = 1.0)
pB = utils.dirichlet_like(B, scale = 1.0)

my_agent = Agent(A = A, B = B, C = C, D = D, pA=pA, pB=pB, save_belief_hist=True, use_param_info_gain=True, policy_len = 4, lr_pA=10000)
...
T = 15 # number of total timesteps
trials = 10# number of trials

for n in range(trials):
    for t in tqdm(range(T)):
        qs_prev = my_agent.qs.copy()
     ...
     #   my_agent.update_B(obs)
    my_agent.reset()
    
### animate results ###
def show_env(grid_dims, loc_list, cue1_location, cue2_locations, reward_locations):
    ...
    for ii, loc_ii in enumerate(cue2_locations):
      ...
      ax.text(column_coord+text_offsets[0], row_coord+text_offsets[1], "l"+str(ii), fontsize = 15, color='k')
    h.set_array(cue_grid.ravel())
    plt.show()

def show_actions_results(my_env, history_of_locs, grid_dims, reward_locations):
    ...
    # animation part
    row, col = all_locations[0, 1], all_locations[0, 0]   # Initial agent's position (row, col)
    img = ax.imshow(
        agent_img,
        extent=(col, col+1, row, row+1),
        zorder=4,
    )
    key_img_to_plot = ax.imshow(
        key_img,
        extent=(cue1_loc[1], cue1_loc[1]+1, cue1_loc[0], cue1_loc[0]+1),
        zorder=3,
    )
    if reward_condition == "BOTTOM":
      cheese_img_to_plot = ax.imshow(
          cheese_img,
          extent=(reward_locations[1][1],reward_locations[1][1]+1,reward_locations[1][0],reward_locations[1][0]+1),
          zorder=3,
      )
    elif reward_condition == "TOP":
      cheese_img_to_plot = ax.imshow(
          cheese_img,
          extent=(reward_locations[0][1],reward_locations[0][1]+1,reward_locations[0][0],reward_locations[0][0]+1),
          zorder=3,
      )

    ax.plot(all_locations[:,1]+0.5,all_locations[:,0]+0.5, 'r', zorder = 1)

    def animate(i):
        col, row = all_locations[i, 1], all_locations[i, 0]   # Initial agent's position (row, col)
        img.set_extent((col, col + 1, row, row + 1))  # Update to new grid cell
        ax.plot(all_locations[:,1]+0.5,all_locations[:,0]+0.5, 'r', zorder = 1)

        return img,

    ax.set_aspect('equal')
    ani = animation.FuncAnimation(fig, animate, frames=len(all_locations), interval=500)
    #plt.show()
    display(ani)
    return ani

a = show_actions_results(my_env, history_of_locs, grid_dims, reward_locations)
plt.show()
