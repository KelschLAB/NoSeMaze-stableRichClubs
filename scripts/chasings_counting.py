...
def boxplot_chasing():
    ...
    chasings_rc = [] # chasings from RC   
    chasings_mutants = [] # chasings from RC   
    chasings_others = [] # chasings from RC   

    for idx, g in enumerate(labels):
        data = read_graph(["C:\\Users\\Corentin offline\\Documents\\GitHub\\clusterGUI\\data\\chasing\\single\\"+g+"_single_chasing.csv"], percentage_threshold = 0)[0]
        for rc in all_rc[idx]:
...
def boxplot_approaches():
    ...
    approaches_rc = [] # chasings from RC   
    approaches_mutants = [] # chasings from RC   
    approaches_others = [] # chasings from RC   

    for idx, g in enumerate(labels):
        data = read_graph(["..\\data\\averaged\\"+g+"\\approaches_resD7_1.csv"], percentage_threshold = 10)[0] + read_graph(["..\\data\\averaged\\"+g+"\\approaches_resD7_2.csv"], percentage_threshold = 10)[0]
        for rc in all_rc[idx]:
