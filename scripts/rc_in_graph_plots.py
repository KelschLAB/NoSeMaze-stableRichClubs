...
def day_to_day_interactions(dirname, mnn = 3, deg = 3):
    """
    ...
    fig, axs = plt.subplots(5, 1, figsize=(5, 15)) #putting all results directly on same figure
    for i in range(5):
        file = "interactions_resD3_"+str(i+1)+".csv"
        display_graph([datapath+file], axs[i], mnn = mnn, node_metric = "rich-club", deg = deg, layout = "circle", node_labels = [])#, idx = [1,1,1,0,0,0,1,1,1,0])
    fig.suptitle(dirname, fontsize=12)
    ...
    plt.savefig("C:\\Users\\wolfgang.kelsch\\Downloads\\"+dirname+"_mnn"+str(mnn)+"_deg"+str(deg)+"_interactions.svg")
    plt.tight_layout()
    plt.show()
...
day_to_day_interactions("G10", mnn = 4, deg = 4)